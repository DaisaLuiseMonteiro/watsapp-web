...
// Chargement des contacts avec le dernier message
async function loadContacts() {
    try {
        ...
        users.forEach(user => {
            const lastMessage = messages.filter(msg => msg.senderId === user.id || msg.receiverId === user.id).pop();
...
async function loadMessages(userId) {
    try {
        ...
        allMessages.filter(msg => msg.senderId === userId || msg.receiverId === userId).forEach(msg => {
